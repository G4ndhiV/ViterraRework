...
/** Modos del recurso "developments" en tokko-sync (ninguno borra desarrollos). */
...
type TokkoSyncResponse = {
  ...
  error?: string;
  summary?: {
    developments?: TokkoDevelopmentsSummary;
  };
};
...
/** Importa desarrollos desde Tokko Broker: solo nuevos, solo actualizar existentes, o ambos. */
export function DevelopmentImportDialog({ open, onOpenChange, onImportComplete }: Props) {
  const [step, setStep] = useState<SyncStep>("idle");
  const [mode, setMode] = useState<ImportMode>("new_only");
  const [summary, setSummary] = useState<TokkoDevelopmentsSummary | null>(null);
  ...
  const handleImport = async () => {
    ...
    const { data, error } = await client.functions.invoke<TokkoSyncResponse>("tokko-sync", {
      // insertOnlyNew acompaña a developmentsMode a propósito: si la Edge Function desplegada
      // fuera una versión vieja que ignora developmentsMode, caería al modo "solo nuevos"
      // (seguro, sin sobrescribir) en lugar del modo completo con borrado.
      body: { resources: ["developments"], developmentsMode: mode, insertOnlyNew: true, dryRun: false },
    });
    ...
    const devsSummary = data.summary?.developments ?? { upserted: 0, errors: [] };
    setSummary(devsSummary);
    setStep("done");

    const createdCount = devsSummary.created ?? devsSummary.upserted;
    const updatedCount = devsSummary.updated ?? 0;
    if (devsSummary.errors.length === 0) {
      if (createdCount === 0 && updatedCount === 0) {
        toast.success("No hay cambios que importar de Tokko");
      } else {
        const parts = [
          createdCount > 0 ? `${createdCount} nuevos` : null,
          updatedCount > 0 ? `${updatedCount} actualizados` : null,
        ].filter(Boolean);
        toast.success(`Importación de Tokko: ${parts.join(" · ")}`);
      }
    } else {
      toast.warning(`${devsSummary.upserted} procesados, ${devsSummary.errors.length} con errores`);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Importar desarrollos de Tokko Broker</DialogTitle>
          <DialogDescription>
            {step === "idle" && "Sincroniza los desarrollos de tu cuenta de Tokko Broker con el sitio."}
          ...
          <div className="space-y-4">
            ...
            {mode !== "new_only" && (
              <div className="flex items-start gap-2 rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-2.5 text-xs text-yellow-900">
                ...
                <p>
                  Al actualizar, los datos de Tokko sobrescriben los del CRM (nombre y unidades
                  incluidos). Si corregiste algo solo en el CRM, primero refléjalo en Tokko para no
                  perderlo. Nunca se eliminan desarrollos.
                </p>
              </div>
            )}
          </div>
        ...
        {step === "syncing" && (
          <div className="space-y-3 py-8 text-center">
            ...
            <p className="text-sm text-slate-600">
              Comparando tus desarrollos con Tokko Broker. Esto puede tardar unos minutos si hay
              muchas unidades.
            </p>
          </div>
        )}
                ...
                <div>
                  ...
                  >
                    {(() => {
                      ...
                      const parts = [
                        created > 0 ? `${created} nuevo${created === 1 ? "" : "s"}` : null,
                        updated > 0 ? `${updated} actualizado${updated === 1 ? "" : "s"}` : null,
                      ].filter(Boolean);
                      return `Desarrollos: ${parts.join(" · ")}`;
                    })()}
                  </p>
                  ...
                  {typeof summary.skippedNew === "number" && summary.skippedNew > 0 && (
                    <p className="mt-1 text-sm text-slate-600">
                      {summary.skippedNew} son nuevos en Tokko y no se agregaron (elige otra opción
                      para importarlos).
                    </p>
                  )}
                </div>
